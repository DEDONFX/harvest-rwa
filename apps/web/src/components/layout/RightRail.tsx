          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg border text-[10px] font-bold"
              ...
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none">
                <defs><linearGradient id="rr-mnt" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#ffffff"/><stop offset="100%" stopColor="#00C896"/></linearGradient></defs>
                {[0,31,62,93,124,155,186,217,248,279,310,341].map((a,i)=>{const r=(a-90)*Math.PI/180,h=i%2===0?4:3,cx=12,cy=12,ir=3.8;return(<rect key={i} x={cx+Math.cos(r)*(ir+h/2)-0.9} y={cy+Math.sin(r)*(ir+h/2)-h/2} width={1.8} height={h} rx={0.3} fill="url(#rr-mnt)" fillOpacity={0.6+i%2*0.3} transform={`rotate(${a},${cx},${cy})`}/>)})}
              </svg>
              Mantle
            </div>
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg border text-[10px] font-bold"
